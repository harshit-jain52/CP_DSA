...
node merge(node a, node b){
	node tmp;
	tmp.sum = (a.sum + b.sum)%M;
	return tmp;
}

void push(int id, int l, int r){
	if(seg[id].lz>1){
		seg[id].sum = (seg[id].sum*seg[id].lz)%M;
		if(l!=r){
			seg[id<<1].lz = (seg[id<<1].lz*seg[id].lz)%M;
			seg[id<<1|1].lz = (seg[id<<1|1].lz*seg[id].lz)%M;
		}
		seg[id].lz=1;
	}
}
...
int main()
{
	...
	while(m--){
		...
		else cout << query(1,0,n-1,l,r-1).sum << endl;
	}
